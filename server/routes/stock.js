...
stockRouter.get('/waivers/:leagueID', (req, res) => {
  ...
  updateWaivers();
  /**
   * call updateStocks (axios call to IEX to get 150 stocks)
   * take those stocks, map and update relevant info within the
   * DB (symbol, companyName, latestPrice)
   * current price per share update for user profile
   * Stock Update:
   * cpps & companyName where ticker = ticker
   *
   * Once stocks are updated need to pull for waivers and update
   * cross-check with users in league to subtract how much each owns
   */
